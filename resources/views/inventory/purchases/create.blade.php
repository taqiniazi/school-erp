                    ...
                    <form method="POST" action="{{ route('inventory.purchases.store') }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" min="0" name="unit_cost" value="0" class="border rounded w-full py-2 px-3" required>
                            </div>
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-2">Vendor</label>
                                <input name="vendor" class="border rounded w-full py-2 px-3">
                            </div>
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-2">Reference No</label>
                                <input name="reference_no" class="border rounded w-full py-2 px-3">
                            </div>
                            <div class="md:col-span-2">
                                <label class="block text-gray-700 text-sm font-bold mb-2">Purchase Date</label>
                                <input type="date" name="purchase_date" class="border rounded w-full py-2 px-3" required>
                            </div>
                        </div>
                        <div class="flex items-center justify-between mt-4">
                            <button class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Save</button>
                            <a href="{{ route('inventory.purchases.index') }}" class="text-blue-600 hover:text-blue-800">Cancel</a>
                        </div>
                    </form>
                </div>
            </div>
