            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex justify-between mb-4">
                        ...
                        <a href="{{ route('accounting.expense.create') }}" class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
                            New Expense
                        </a>
                    </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($expenses as $row)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right font-bold text-red-600">{{ number_format($row->amount, 2) }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right">
                                            <a href="{{ route('accounting.expense.edit', $row) }}" class="text-blue-600 hover:text-blue-800 mr-3">Edit</a>
                                            <form action="{{ route('accounting.expense.destroy', $row) }}" method="POST" class="inline" onsubmit="return confirm('Delete this expense?')">
                                                @csrf
                                                @method('DELETE')
