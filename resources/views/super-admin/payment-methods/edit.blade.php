        ...
        <div class="card-body">
            <form action="{{ route('super-admin.payment-methods.update', $paymentMethod) }}" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                            <option value="manual" {{ old('type', $paymentMethod->type) == 'manual' ? 'selected' : '' }}>Manual (Bank Transfer, Cash, etc.)</option>
                            <option value="gateway" {{ old('type', $paymentMethod->type) == 'gateway' ? 'selected' : '' }}>Payment Gateway (Stripe, PayPal)</option>
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="mb-3" id="instructions_group">
                    ...
                    <textarea class="form-control @error('instructions') is-invalid @enderror" id="instructions" name="instructions" rows="3">{{ old('instructions', $paymentMethod->instructions) }}</textarea>
                ...
                <div class="mb-3" id="account_details_group">
                    ...
                    <div id="account_details_container">
                        ...
                        @if(old('account_details'))
                            ...
                            @endforeach
                        @elseif($paymentMethod->account_details && is_array($paymentMethod->account_details))
                             @foreach($paymentMethod->account_details as $key => $value)
                                <div class="input-group mb-2 account-detail-row">
                                    <input type="text" class="form-control" name="account_details_keys[]" value="{{ $key }}" placeholder="Key (e.g. Bank Name)">
                                    <input type="text" class="form-control" name="account_details_values[]" value="{{ $value }}" placeholder="Value (e.g. HBL)">
                                    <button type="button" class="btn btn-danger remove-detail"><i class="fas fa-trash"></i></button>
