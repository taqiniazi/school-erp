                    ...
                    <form action="{{ route('payroll.allowances.update', $allowance) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="name" value="{{ $allowance->name }}" class="shadow appearance-none border rounded w-full py-2 px-3" required>
                            </div>
                            <div class="mb-4 flex items-center">
                                <input type="checkbox" name="is_percentage" id="is_percentage" {{ $allowance->is_percentage ? 'checked' : '' }} class="mr-2">
                                <label for="is_percentage" class="text-gray-700 text-sm font-bold">Percentage</label>
                            </div>
                            <div class="mb-4">
                                <label class="block text-gray-700 text-sm font-bold mb-2">Amount / %</label>
                                <input type="number" step="0.01" name="amount" value="{{ $allowance->amount }}" class="shadow appearance-none border rounded w-full py-2 px-3" required>
                            </div>
                            <div class="mb-4 flex items-center">
                                <input type="checkbox" name="is_active" id="is_active" {{ $allowance->is_active ? 'checked' : '' }} class="mr-2">
